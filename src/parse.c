...
/*   Updated: 2017/11/13 11:47:37 by alucas-          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_u08	fillit_validate(t_tetr *tetr)
{
	t_i08 x;
	t_i08 y;
	t_u08 c;
	...
	while (++x < 4 && (y = -1) < 0)
		while (++y < 4)
			...
			{
				++tetr->n;
				if (x < 3 && BIT_AT(tetr->form, x + 1, y, 4))
					++c;
				if (y < 3 && BIT_AT(tetr->form, x, y + 1, 4))
					++c;
			}
	return ((t_u08)(c < 3));
}
...
static t_u08	fillit_parse(t_ctx *ctx, t_dstr *str)
{
	...
	if (!(ctx->n /= 21) || ctx->n > 26)
		return (1);
	if (!(ctx->tetrs = malloc(ctx->n * sizeof(t_tetr))))
		return (1);
	buf = str->buf;
	tetrs = ctx->tetrs;
	while (*buf)
